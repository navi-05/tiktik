                    ...
                    {searchedAccounts?.length > 0 ? (
                        searchedAccounts.map((user: IUser, i: number) => (
                            <Link href={`/profile/${user._id}`} key={i}>
                                <div className='flex items-center w-fit gap-3 mt-3 pb-2 border-b-2 border-gray-100 cursor-pointer font-semibold'>
                                    <div>
                                    <Image 
                                        src={user.image}
                                        width={50}
                                    ...
                                    <p className='flex gap-1 cursor-pointer items-center text-[18px] font-bold leading-6 text-primary lowercase'>
                        ...
                        <NoResults text={`No accounts related with ${searchTerm}`} /> 
                    )}
                    </div>
            ) : (
                <div className='md:mt-16 flex flex-wrap gap-6 md:justify-start'>
                    <p className='text-gray-400 -mt-10'>Showing results for <span className='text-lg font-semibold text-[#f51997]'>{searchTerm}</span></p>
                    {videos?.length ? (
